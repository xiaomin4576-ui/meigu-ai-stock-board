...
- 基本面因子(共识/评级/估值PEG)全缺 → 卡片标"**技术分·仅技术面**"(美股免费档现状);可算因子 ≤3 → "**数据不足·暂不评分**"。
...
- **云端共识根治(唯一需用户配 key 的项)**:接 Finnhub/FMP 免费档(price-target + recommendation + earnings,云端 IP 可达、非 Yahoo 黑名单)补回**美股共识/评级/财报日** → 美股从"技术分(5/9)"升级为完整"评分(8-9/9)",只需 1 个免费 key(env 加 `FINNHUB_KEY`)。
