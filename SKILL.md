...
3. **均值回归 / 回调买入(2026-07-14 收盘数据回测后精化)**:买点=回踩【现价下方最近的有效结构支撑】,按现价相对均线择锚——价>MA20→锚MA20一线(强势浅回调,最易触及);MA50<价≤MA20→锚MA50;价≤MA50(破位)→锚MA200/前低深回调。**硬约束:买区上沿必须≤现价**(高于现价=追涨)。**观望≠留空**:观望也要给"等回踩到买区+回补前高的目标",只有前高hi≤现价×1.05(高位无空间)或数据不足(因子≤3)才可给"—"。目标须相对现价成立(下沿>现价、中值≥现价×1.15)。★教训:早期"强势股一律锚MA20"是错的——股票跌破MA20后MA20在现价上方,锚它=追涨;必须锚"现价下方"最近支撑。见 [[data-integrity-rules]] 第25条。
