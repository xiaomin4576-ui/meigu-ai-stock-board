...
def validate_call(tk, c, s):
    ...
    if viol:
        c = dict(c)
        # 原 th 截断到 70 字会断在逗号成残句(审计抓到 MRVL/中芯等)→ 改完整句,不截断
        c.update({"sig": "观望", "buy": "—", "tgt": "—", "ret": "—",
                  "th": f"⚠️ 触发护栏({viol}):本期暂不给具体买卖点,待价格回踩有效结构支撑、风险补偿到位后再评估。"})
    return c, viol
