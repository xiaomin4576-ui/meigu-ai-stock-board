...
def _age_str(c):
    """候选发布距今天数标签(供 prompt 行首,让模型识别旧事件)。age_days 由 fetch_news 打;缺失则现算。"""
    a = c.get("age_days")
    if a is None:
        import email.utils
        s = str(c.get("ts") or "").strip()
        m = re.search(r"(20\d{2})-(\d{2})-(\d{2})", s)
        try:
            dd = datetime.date(int(m.group(1)), int(m.group(2)), int(m.group(3))) if m else email.utils.parsedate_to_datetime(s).date()
            a = (datetime.date.fromisoformat(TODAY) - dd).days
        except Exception:
            a = None
    return f"{a}天前" if a is not None else "日期未知"
...
def main():
    ...
    for i, c in enumerate(pool):
        ...
        lines.append(f"{i+1}. [{c.get('source','')}|发布{_age_str(c)}] {c.get('title','')} — {str(c.get('summary',''))[:100]}")
    ...
    if dropped and len(items) < 15:
        ...
        if left:
            supp_lines = []
            for i, c in left:
                supp_lines.append(f"{i+1}. [{c.get('source','')}|发布{_age_str(c)}] {c.get('title','')} — {str(c.get('summary',''))[:100]}")
